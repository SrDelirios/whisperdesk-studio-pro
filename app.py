...
def get_hardware_status():
    """Retorna información en tiempo real sobre la GPU (CUDA), Ollama y Gemini con caché ligero."""
    now = time.time()
    if _HW_CACHE["data"] is not None and (now - _HW_CACHE["last_updated"]) < 10:
        return jsonify(_HW_CACHE["data"])
    ...
    structured = MeetingActaSummarizer.get_structured_models(gemini_key=gemini_key)
    ...
    res_data = {
        ...
        "ai_models": models,
        "structured_models": structured
    }
    _HW_CACHE["data"] = res_data
    _HW_CACHE["last_updated"] = now

    return jsonify(res_data)

@app.route('/api/gemini/test', methods=['POST'])
def test_gemini_api():
    """Prueba si una API key de Google Gemini es válida consultando la lista de modelos activos."""
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            count = len([m for m in models_data if 'generateContent' in m.get('supportedGenerationMethods', [])])
            return jsonify({"success": True, "message": f"Conexión con Google AI Studio exitosa ({count} modelos activos detectados)"})
        else:
            err_msg = r.text
            try:
                ...
                pass
            return jsonify({"success": False, "error": f"Error de autenticación ({r.status_code}): {err_msg}"}), 400
    except Exception as e:
        return jsonify({"success": False, "error": f"Error de red al conectar con Google AI Studio: {str(e)}"}), 500
...
def export_acta_docx():
    """Genera y descarga un documento de Word (.docx) formateado a partir del Markdown del acta."""
    data = request.json or {}
    markdown_text = data.get('markdown', '').strip()
    title = data.get('title', 'Acta_Oficial_de_Sesion')
    if not markdown_text:
        return jsonify({"error": "No hay texto para exportar"}), 400

    try:
        docx_io = MeetingMinutesExporter.generate_docx(markdown_text, title=title)
        safe_name = make_safe_filename(title) or "Acta_Oficial"
...
def export_acta_pdf():
    """Genera y descarga un archivo PDF ejecutivo formateado a partir del Markdown del acta."""
    data = request.json or {}
    markdown_text = data.get('markdown', '').strip()
    title = data.get('title', 'Acta_Oficial_de_Sesion')
    if not markdown_text:
    ...
    try:
        pdf_io = MeetingMinutesExporter.generate_pdf(markdown_text, title=title)
        safe_name = make_safe_filename(title) or "Acta_Oficial"
...
def export_acta_md():
    """Descarga el archivo Markdown (.md) del acta."""
    data = request.json or {}
    markdown_text = data.get('markdown', '').strip()
    title = data.get('title', 'Acta_Oficial_de_Sesion')
    if not markdown_text:
    ...
    mem_io = io.BytesIO(markdown_text.encode('utf-8'))
    mem_io.seek(0)
    return send_file(
        mem_io,
